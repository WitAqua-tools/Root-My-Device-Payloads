...
| `xig07-jp-OS3.0.7.0.WNEJPKD` | `core61` | Xiaomi 14T (au XIG07) | MediaTek MT6897 | JP | `OS3.0.7.0.WNEJPKD` | `6.1.138-android14-11-g44bda9e8f6e9-ab13792638` (`android14-6.1`, 4K pages) | `Xiaomi/XIG07_jp_kdi/XIG07:16/BP2A.250605.031.A3/OS3.0.7.0.WNEJPKD:user/release-keys` | The exploit core is device-verified through the non-app build: `done=1 root=1` with the su daemon serving, from a settled boot, shipped defaults, on the first attempt -- 5 runs in 7 (two of which were second runs on a boot an earlier run had left alive; 4 in 5 counting only first runs), the other 2 panicking in the punch before the R/W, so a miss costs a reboot rather than a retry. After a run that rooted, the next one fails on the per-uid pipe budget its resident helpers still hold. Everything the application does after it has a payload is verified from adb as well -- the release `.so` through the helper's `--run-payload` runner, ksud staged, `--late-load android14-6.1` answering, and the manager showing the LKM Jailbreak-mode card at 32525-2 with SELinux back to Enforcing -- and this is the first android14-6.1 KernelSU build here to be loaded anywhere. The application's own route works as of the boot-id slide source: two runs from the app on two clean Enforcing boots, first attempt each, uid 10325 in `u:r:untrusted_app:s0`, `done=1 root=1` and the manager at LKM Jailbreak-mode 32525-2. tracefs is still `EACCES` from that domain; it is simply no longer fatal. `kernelVersion` is set from the device's own `/proc/version`, so this target is in the feed and its KernelSU build is in the job a release waits on -- but the *feed path* itself has never run for it: every run so far read a local folder through the app's debug mode, and CI has never built this target's app payload or its android14-6.1 KernelSU pair as release assets. |
