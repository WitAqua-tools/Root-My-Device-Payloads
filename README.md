...
| `xig07-jp-OS3.0.7.0.WNEJPKD` | `core61` | Xiaomi 14T (au XIG07) | MediaTek MT6897 | JP | `OS3.0.7.0.WNEJPKD` | `6.1.138-android14-11-g44bda9e8f6e9-ab13792638` (`android14-6.1`, 4K pages) | `Xiaomi/XIG07_jp_kdi/XIG07:16/BP2A.250605.031.A3/OS3.0.7.0.WNEJPKD:user/release-keys` | The exploit core is device-verified through the non-app build: `done=1 root=1` with the su daemon serving, from a settled boot, shipped defaults, on the first attempt -- 4 runs in 6, the other 2 panicking in the punch before the R/W, so a miss costs a reboot rather than a retry. A second run on the same boot fails on the per-uid pipe budget the first one still holds. The application route is not wired here: core61 leaks the slide from tracefs, which an app's SELinux domain cannot read, so `kernelVersion` is null and the target stays out of the feed deliberately. |
